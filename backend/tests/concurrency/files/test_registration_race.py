"""
Tests what happens when multiple identical registration requests
arrive at the exact same time — a real scenario when a user double-clicks
submit, or a script/bot retries too fast.
"""
import asyncio
import uuid
...
async def _register(client_factory, email: str) -> int:
...
async def test_concurrent_registration_same_email_only_one_succeeds(
    ...
    assert conflict_count == 4, (
        f"expected 4 conflicts, got {conflict_count}: {results}"
    )
