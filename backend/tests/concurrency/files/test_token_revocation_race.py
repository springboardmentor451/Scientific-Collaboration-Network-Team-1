...
from conftest import auth_headers_for, make_active_user


async def _access_protected_route(client_factory, email: str) -> int:
    async with client_factory() as client:
        res = await client.get("/api/researchers/me", headers=auth_headers_for(email))
        return res.status_code


async def _double_logout(client_factory, refresh_token: str, access_token: str) -> int:
    async with client_factory() as client:
...
async def test_concurrent_logout_calls_same_token_idempotent(
    ...
    should not crash the server on the second call even though the
