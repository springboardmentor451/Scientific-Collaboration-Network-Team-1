...
services/collaboration.py performs:
    existing.collaboration_count += 1
    await self.session.commit()

This is a classic read-modify-write race. If N requests read the same
starting count simultaneously, all N compute count+1 independently and
the final stored value is count+1, NOT count+N. Updates are silently lost.

This test proves the bug exists (if unfixed) or proves the fix works
(atomic UPDATE ... SET count = count + 1 at the database level).
"""
...
async def _create_collaboration(
    client_factory, email: str, researcher_ids: list[int]
) -> dict:
...
async def test_collaboration_count_survives_concurrent_increments(
    ...
    """
    Two researchers collaborate 5 times "simultaneously" (e.g. both submit
    a joint paper form at once, or a batch import runs in parallel).
