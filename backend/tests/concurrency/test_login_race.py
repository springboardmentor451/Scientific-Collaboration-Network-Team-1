"""
Tests what happens when the same user logs in from multiple devices/tabs at the same moment,
each login call generates a new OTP, overwriting the previous one.
This tests that behaviour is at least consistent and does not corrupt state or crash.
"""
...
from tests.concurrency.conftest import make_active_user

PASSWORD = "TestPass123"


async def _login(client_factory: Callable[[], AsyncClient], email: str) -> int:
...
async def test_concurrent_login_wrong_password_all_rejected(
    ...
    async def _login_wrong(client_factory: Callable[[], AsyncClient]) -> int:
        async with client_factory() as client:
            res: Response = await client.post(
                "/api/auth/login",
    ...
    results: list[int] = await asyncio.gather(
        *[_login_wrong(client_factory) for _ in range(5)]
    )
...
async def test_concurrent_login_only_latest_otp_valid(
    ...
    """
    After N concurrent logins overwrite the OTP record, only the LAST one written should be verifiable.
    This documents actual behaviour so a regression (e.g. multiple valid OTPs at once) gets caught.
    """
    user: User = await make_active_user(session)
    await asyncio.gather(*[_login(client_factory, user.email) for _ in range(3)])
    records: Sequence[VerificationCode] = (
